 ...
 */
const mmap_region_t plat_sigi_mmap[] = {
	MAP_REGION_FLAT(DEVICE0_BASE, DEVICE0_SIZE, MT_DEVICE | MT_RW | MT_SECURE),
	MAP_REGION_FLAT(DEVICE1_BASE, DEVICE1_SIZE, MT_DEVICE | MT_RW | MT_SECURE),
	{ 0 }
};

const mmap_region_t *plat_sigi_get_mmap(void)
{
	return plat_sigi_mmap;
}

unsigned int plat_get_syscnt_freq2(void)
{
	return SIGI_OSC24M_CLK_IN_HZ;
}
...
static void sigi_print_platform_name(void)
{
	NOTICE("ATF running on %s\n", PLATFORM_NAME);
}

void sigi_config_setup(void)
{
	sigi_print_platform_name();
