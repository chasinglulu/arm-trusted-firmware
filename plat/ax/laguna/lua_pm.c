...
#define lua_make_pwrstate_lvl0(lvl0_state, pwr_lvl, type) \
...
#define lua_make_pwrstate_lvl1(lvl1_state, lvl0_state, pwr_lvl, type) \
		(((lvl1_state) << LUA_LOCAL_PSTATE_WIDTH) | \
		 lua_make_pwrstate_lvl0(lvl0_state, pwr_lvl, type))
 ...
 */
static const unsigned int lua_pm_idle_states[] = {
	/* State-id - 0x01 */
	lua_make_pwrstate_lvl1(LUA_LOCAL_STATE_RUN, LUA_LOCAL_STATE_RET,
				MPIDR_AFFLVL0, PSTATE_TYPE_STANDBY),
	/* State-id - 0x02 */
	lua_make_pwrstate_lvl1(LUA_LOCAL_STATE_RUN, LUA_LOCAL_STATE_OFF,
				MPIDR_AFFLVL0, PSTATE_TYPE_POWERDOWN),
	/* State-id - 0x22 */
	lua_make_pwrstate_lvl1(LUA_LOCAL_STATE_OFF, LUA_LOCAL_STATE_OFF,
				MPIDR_AFFLVL1, PSTATE_TYPE_POWERDOWN),
	0,
};

/**
 * lua_validate_power_state() - Platform handler to check if power state is valid
 *
 ...
 */
int lua_validate_power_state(unsigned int power_state,
			    psci_power_state_t *req_state)
{
	 ...
	 */
	for (i = 0; !!lua_pm_idle_states[i]; i++) {
		if (power_state == lua_pm_idle_states[i])
			break;
	}

	/* Return error if entry not found in the idle state array */
	if (!lua_pm_idle_states[i])
	...
	while (state_id) {
		req_state->pwr_domain_state[i++] = state_id &
						LUA_LOCAL_PSTATE_MASK;
		state_id >>= LUA_LOCAL_PSTATE_WIDTH;
	}
...
/**
 * lua_validate_ns_entrypoint() - Platform handler to check ns_entrypoint
 *
 ...
 * Return: PSCI_E_SUCCESS if success, error code otherwise.
 */
static int lua_validate_ns_entrypoint(uintptr_t entrypoint)
{
	/*
	 * Check if the non secure entrypoint lies within the non
	 * secure DRAM.
	 */
	if ((entrypoint >= LUA_DRAM_BASE) &&
		(entrypoint < (LUA_DRAM_BASE + LUA_NS_DDR_SIZE)))
		return PSCI_E_SUCCESS;
...
/**
 * lua_cpu_standby() - Put CPU to standy mode
 *
 ...
 * Return: void
 */
void lua_cpu_standby(plat_local_state_t cpu_state)
{
	assert(cpu_state == LUA_LOCAL_STATE_RET);
 ...
 * Return: PSCI_E_SUCCESS if success, error code otherwise
 */
static int lua_pwr_domain_on(u_register_t mpidr)
{
...
/**
 * lua_pwr_domain_off() - Power off a cpu.
 *
 ...
 * Return: void
 */
static void lua_pwr_domain_off(const psci_power_state_t *target_state)
{
	lua_pwr_gic_off();
}

/**
 * lua_pwr_domain_suspend() - Put system into suspend state.
 *
 ...
 * Return: void
 */
static void lua_pwr_domain_suspend(const psci_power_state_t *target_state)
{
	assert(0);
}
...
/**
 * lua_pwr_down_wfi() - Powerdown the CPU
 *
 * @target_state: Platform coordinated target local power states
 *
 ...
 * Return: void
 */
static void __dead2 lua_pwr_down_wfi(const psci_power_state_t *target_state)
{
	disable_mmu_el3();
...
/**
 * lua_pwr_domain_on_finish() - Platform handler when cpu is powered on.
 *
 * @target_state: Platform coordinated target local power states
 *
 ...
 * Return: void
 */
static void lua_pwr_domain_on_finish(const psci_power_state_t *target_state)
{
	assert(target_state->pwr_domain_state[MPIDR_AFFLVL0] ==
					LUA_LOCAL_STATE_OFF);

	lua_pwr_gic_on_finish();
}

/**
 * lua_pwr_domain_suspend_finish() - Platform handler after wakeup.
 *
 * @target_state: Platform coordinated target local power states
 *
 ...
 * Return: void
 */
static void lua_pwr_domain_suspend_finish(const psci_power_state_t *target_state)
{
	assert(0);
}

/**
 * lua_system_off() - Platform handler to poweroff the system.
 *
 ...
 * Return: void
 */
static void __dead2 lua_system_off(void)
{
	panic();
}

/**
 * lua_system_reset() - Platform handler to reboot the system.
 *
 ...
 * Return: void
 */
static void __dead2 lua_system_reset(void)
{
	panic();
}

/**
 * lua_psci_ops - j6 platform psci ops
 */
static plat_psci_ops_t lua_psci_ops = {
	.cpu_standby = lua_cpu_standby,
	.pwr_domain_on = lua_pwr_domain_on,
	.pwr_domain_off = lua_pwr_domain_off,
	.pwr_domain_suspend = lua_pwr_domain_suspend,
	.pwr_domain_pwr_down_wfi = lua_pwr_down_wfi,
	.pwr_domain_on_finish = lua_pwr_domain_on_finish,
	.pwr_domain_suspend_finish = lua_pwr_domain_suspend_finish,
	.system_off = lua_system_off,
	.system_reset = lua_system_reset,
	.validate_power_state = lua_validate_power_state,
	.validate_ns_entrypoint = lua_validate_ns_entrypoint,
};
...
{
	uintptr_t *mailbox = (void *)PLAT_LUA_TRUSTED_MAILBOX_BASE;

	*mailbox = sec_entrypoint;
	secure_entrypoint = (unsigned long)sec_entrypoint;

	assert(psci_ops);
	*psci_ops = &lua_psci_ops;
