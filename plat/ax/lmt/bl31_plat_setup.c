 ...
 */
const mmap_region_t plat_lmt_mmap[] = {
	...
	MAP_REGION_FLAT(LMT_PMU_BASE, 0x1000, MT_DEVICE | MT_RW | MT_SECURE),
	{ 0 }
};

const mmap_region_t *plat_lmt_get_mmap(void)
{
	return plat_lmt_mmap;
}

unsigned int plat_get_syscnt_freq2(void)
{
	return LMT_OSC24M_CLK_IN_HZ;
}
...
static void lmt_print_platform_name(void)
{
	NOTICE("ATF running on %s\n", PLATFORM_NAME);
}

void lmt_config_setup(void)
{
	lmt_print_platform_name();
...
void bl31_plat_arch_setup(void)
{
	...
	setup_page_tables(bl_regions, plat_lmt_get_mmap());
	enable_mmu_el3(0);
}
